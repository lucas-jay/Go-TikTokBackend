...
func IsFollowing(HostId uint, GuestId uint) bool {
	...
	//判断关注是否存在
	if err := common.DB.Model(&model.Following{}).
...
func CreateFollowing(HostId uint, GuestId uint) error {
	...
	//2.新建following
	if err := common.DB.Model(&model.Following{}).Create(&newFollowing).Error; err != nil {
		return err
	}
	return nil
}
...
func DeleteFollowing(HostId uint, GuestId uint) error {
	...
	//2.删除following
	if err := common.DB.Model(&model.Following{}).Where("host_id=? AND guest_id=?", HostId, GuestId).Delete(&deleteFollowing).Error; err != nil {
		return err
	}
...
func FollowAction(HostId uint, GuestId uint, actionType uint) error {
	...
	if actionType == 1 {
		...
		if IsFollowing(HostId, GuestId) {
			...
			err1 := common.DB.Transaction(func(db *gorm.DB) error {
	...
	if actionType == 2 {
		...
		if IsFollowing(HostId, GuestId) {
			...
			if err1 := common.DB.Transaction(func(db *gorm.DB) error {
